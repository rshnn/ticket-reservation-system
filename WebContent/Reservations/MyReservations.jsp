...
<body>
	...
	<div align='right'>
		<a href="../HomePages/CustomerHome.jsp">Back to Manager Home</a>
	</div>
	<br>
	...
	<%
		...
		String command = "SELECT resNo, dateReserved, group_concat(distinct concat(airlineID, flightNumber)) flightinfo, dominter, "
				+ "passengers, group_concat(ticketID) ticketID, group_concat(seatNo) seatNos, group_concat(meal) meal, totalFare, bookingFee, type "
				+ "from Purchases " + "join Reservations using (resNo) " + "join Tickets using (ticketID) "
				+ "join Users using (username) " + "join Flight_operates using (airlineID, flightNumber) "
				+ "WHERE username='" + user + "'" + "group by resNo";
